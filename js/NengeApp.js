let NengeApp = new class {
    ...
    btnMap = {
        ...
        'translate':{
            ...
            'show':e=>{
                let translate = this.translateKey||{},
                    HTML = `<div class="gba-list-translate"><h3>翻译API 目前使用百度</h3><p>申请地址:<a href="https://api.fanyi.baidu.com/product/22">https://api.fanyi.baidu.com/product/22</a></p><p>申请成功后点击<a href="https://api.fanyi.baidu.com/manage/developer">开发者信息</a></p>`
                    +`<p><label>APPID:<input type="text" class="gba-translate-id" tabIndex="0" value="${translate.id||''}"></label></p>`
                    +`<p><label>密 钥：<input type="text" class="gba-translate-key" tabIndex="1" value="${translate.key||''}"></label></p>`
                    +`<p><label>来 源：<input type="text" class="gba-translate-from" tabIndex="1" value="${translate.from||'en'}"></label></p>`
                    +`<p><label>目 标：<input type="text" class="gba-translate-to" tabIndex="1" value="${translate.to||'zh'}"></label></p>`
                    +`<p><label>跨域：<input type="text" class="gba-translate-host" tabIndex="1" value="${translate.host||'https://api.nenge.net/translateBaidu.php'}"></label><br>https://api.nenge.net/translateBaidu.php</p><p><label><button data-btn="translate-save">保存</button></p><p>跨域服务器由能哥网自由服务提供（使用期限不确定！），如果有服务器资源的朋友，可以自己搭建下方是PHP代码！百度翻译月免费1万次，多出部分四毛钱一次！</p>`
            ...
            Baidu: (method, config) => {
                ...
                let g = new FormData(),
                    ...
                    gd = {
                        ...
                        "from": this.translateKey.from,
                        "to": this.translateKey.to,
                        "q":this.translateKey.q||"",
                        "salt": Math.random(),
                        "cuid": 'APICUID',
                        "mac": 'mac',
                        "version": 3,
